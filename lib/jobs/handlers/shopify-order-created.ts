...
export const handleShopifyOrderCreated: JobHandler = async ({
  ...
  const insert = await admin
    ...
    .insert({
      ...
      source_name: "shopify.mock",
      metadata: {
        demo: true,
        demo_seed: data.demo_seed ?? null,
        job_id: job.id,
        event: "shopify.order.created.mock",
      } as Json,
      tags: ["jobs", "shopify", "mock"],
    })
    .select("id")
    .single();
  ...
  if (insert.error || !insert.data) {
    ...
    throw new PermanentJobError("DATABASE_ERROR", "No se pudo crear el pedido mock.");
  }
