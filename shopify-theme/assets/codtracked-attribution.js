/**
 * CODTracked storefront attribution (theme asset — no Vercel host required)
 *
 * Install in Shopify theme:
 * 1. Copy this file to theme folder: assets/codtracked-attribution.js
 * 2. In layout/theme.liquid, before </head>, add:
 *    <script src="{{ 'codtracked-attribution.js' | asset_url }}" defer></script>
 * 3. Publish / push the theme, then test with UTM URL + Add to cart.
 *
 * Writes utm_* / click IDs to cart attributes → order note_attributes.
 */
(function () {
  "use strict";
  ...
  function syncCartAttributes() {
    ...
    fetch("/cart.js", { credentials: "same-origin" })
      ...
      .then(function (cart) {
        ...
        for (var k in merged) {
          if (
            ...
            String(merged[k] || "") !== String(existing[k] || "")
          ) {
            changed = true;
            break;
          }
      ...
      .catch(function () {
        /* password gate / checkout surfaces */
      })
      .finally(function () {
        syncing = false;
      });
  ...
  if (
    ...
    typeof window.Shopify.customerPrivacy.addEventListener === "function"
  ) {
