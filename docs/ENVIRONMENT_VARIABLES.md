...
| `STRIPE_PRICE_STARTER_MONTH` / `_YEAR` | Server only | Fallback **live** Stripe Price IDs when `plan_provider_prices` has no row. Same for `GROWTH` and `SCALE`. |
| `STRIPE_TEST_SECRET_KEY` | Server only | Stripe **test** secret (`sk_test_…`). Enables Facturación “Modo test” for allowlisted emails. |
| `STRIPE_TEST_WEBHOOK_SECRET` | Server only | Test-mode webhook signing secret (`whsec_…`). Same URL as live. |
| `STRIPE_TEST_PRICE_*` | Server only | Test Price IDs (`STRIPE_TEST_PRICE_STARTER_MONTH`, etc.). Required for test checkout (DB map is live-only). |
